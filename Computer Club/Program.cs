...
namespace Computer_Club
{
    ...
    class ComputerClub
    {
        ...
        public void Work()
        {
            while (_clients.Count > 0)
            {
                ...
                Console.WriteLine($"У нас новый клиент, и он хочет купить {newClient.DesiredMinutes} минут(ы).");
                ShowAllComputersState();

                Console.Write("\nВы предлагаете клиенту компьютер под номером:");
                ...
                else
                {
                    ...
                    Console.WriteLine("Неверный ввод! Повторите снова. Работают только численные значения. Пока вы разбирались - клиент разозлился и ушёл.");
                }
    ...
    class Computer
    {
        ...
        {
            if (IsTaken)
                Console.WriteLine($"Компьютер занят, оставшее время пользователя: {_minutesRemaining} минут.");
            else
